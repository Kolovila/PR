...
function SWEP:CustomOnPrimaryAttack_BeforeShoot()
	...
	if IsValid(phys) then
		if self:GetOwner():IsPlayer() then
			...
			phys:SetVelocity(self:GetOwner():CalculateProjectile("Line", self:GetNW2Vector("VJ_CurBulletPos"), self:GetOwner():GetEnemy():GetPos() + self:GetOwner():GetEnemy():OBBCenter(), 5000))
		end
	end
